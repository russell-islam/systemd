...
static int save_external_coredump(
        ...
        if (arg_compress) {
                ...
                *ret_size = (uint64_t) uncompressed_size;      /* uncompressed */
...
static int submit_coredump(
        ...
        if (coredump_fd != -1 && coredump_size <= arg_process_size_max) {
        ...
        } else if (coredump_size > arg_process_size_max)
                log_debug("Not generating stack trace: core size %"PRIu64" is greater than %"PRIu64" (the configured maximum)",
                          coredump_size, arg_process_size_max);
        else
                log_debug("Not generating stack trace: uncompressed core unavailable.");
